...
workers.gatherAllChecks = function() {
  ...
  _data.list('checks', function(err, checks) {
    if (!err && checks && checks.length) {
      ...
      debug('Error: could not found any checks to process');
    }
  });
};
  ...
  _data.update('checks', newCheckData.id, newCheckData, function(err) {
    if (!err) {
      ...
      if (alertWarranted) {
        ...
        debug('Nothing changed in the check ' + newCheckData.id);
      }
    } else {
      debug('Error: trying to save updates to check ' + newCheckData.id);
    }
  })
};
          ...
          if (!err) {
            ...
            _logs.truncate(logId, function(err) {
              if (!err) {
                ...
                debug('Could not truncate log file '+ logId, err);
              }
            });
          } else {
            debug('Could not compress log file ' + logId, err)
          }
        })
      });
    } else {
